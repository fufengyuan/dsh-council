  ...
  factory: function(require) {
    ...
    function DagContent(props) {
      ...
      return e('div', { style: { display: 'flex', flexDirection: 'column', height: '100%' } },
        ...
        e('div', {
          ...
          onPointerDown: function(ev) { if (ev.button !== 0 && ev.pointerType === 'mouse') return; dragRef.current = { sx: ev.clientX, sy: ev.clientY, ox: view.x, oy: view.y, moved: false }; try { ev.currentTarget.setPointerCapture(ev.pointerId); } catch(_) {} },
          onPointerMove: function(ev) { var d = dragRef.current; if (!d) return; var dx = ev.clientX - d.sx, dy = ev.clientY - d.sy; if (!d.moved && (Math.abs(dx) > 3 || Math.abs(dy) > 3)) { d.moved = true; userInteractedRef.current = true; } if (d.moved) { panMovedRef.current = true; setView({ x: d.ox + dx, y: d.oy + dy, k: view.k }); } },
          onPointerUp: function() { var d = dragRef.current; dragRef.current = null; if (d && d.moved) { setTimeout(function() { panMovedRef.current = false; }, 0); } },
          ...
          e('svg', { width: '100%', height: '100%', style: { display: 'block', position: 'absolute', top: 0, left: 0 } },
            e('g', { transform: 'translate(' + view.x + ',' + view.y + ') scale(' + view.k + ')' },
              ...
              e('g', { key: 'node-run_root',
                ...
                onClick: function() { if (panMovedRef.current) return; if (rootClickable) props.openSession(runObj.sessionId); }
              },
              ...
              nodes.map(function(n) {
                ...
                return e('g', {
                    ...
                    onClick: function() { if (panMovedRef.current || !clickable) return; if (props.openChildSession) props.openChildSession(parentSid, child); else props.openSession(child.id); }
                  },
    ...
    function CouncilPanelContent(props) {
      ...
      function createIssue() {
        ...
        setIssueErr('');
        setCreating(true);
        // 先在新工作区创建空白会话（一个议题一个会话，不污染当前会话），并注入 /council 触发辩论
        var runSessionId = props.sessionId || '';
        var createPromise;
        if (typeof props.createSession === 'function' && workspaceId) {
          createPromise = Promise.resolve(props.createSession(workspaceId, t)).then(function(res) {
      ...
      return e('div', { style: { flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' } },
        ...
        e('div', { style: { flex: 1, display: 'flex', minHeight: 0, overflow: 'hidden' } },
          ...
          e('div', { style: { flex: 1, overflow: 'auto', padding: '16px 20px' } },
            selectedRunId
              ? e(DagContent, { runId: selectedRunId, sessionId: props.sessionId, useSessions: props.useSessions, openSession: props.openSession, openChildSession: props.openChildSession, hydrateSubagents: props.hydrateSubagents })
      ...
      ctx.slots.inject('shell.overlay', function* () {
        yield ctx.slots.register({
          ...
          inject: function() { return {
            ...
            // 创建新会话并注入 /council 命令，让 agent 自动启动辩论
            createSession: async function(workspaceId, problem) {
              var newId = await ctx.sessions.create({ workspaceId: workspaceId });
              try {
                var binding = ctx.sessions.binding(newId);
                var sess = binding && (binding.session || binding);
                if (sess && typeof sess.prompt === 'function') {
                  await sess.prompt([{ type: 'text', text: '/council --quick ' + problem }], 'queue');
                } else {
                  console.error('[council] prompt unavailable for', newId, 'sess=', sess && Object.keys(sess));
                }
              } catch (e) {
                console.error('[council] prompt inject failed:', e && e.message ? e.message : e);
                // 注入失败不阻塞会话创建；用户可手动在新会话发 /council
              }
              return newId;
            },
