  ...
  factory: function(require) {
    ...
    function DagContent(props) {
      ...
      var edges = (function () {
        ...
        var maxRound = {};
        nodes.forEach(function(n){ if (n.kind !== 'system') { var d = String(n.id).lastIndexOf('-r'); var mid = d > 0 ? n.id.slice(0, d) : n.id; var r = n.round || 0; if ((maxRound[mid] || 0) < r) maxRound[mid] = r; } });
        function lastRound(n) { if (!n || n.kind === 'system') return false; var d = String(n.id).lastIndexOf('-r'); var mid = d > 0 ? n.id.slice(0, d) : n.id; return (n.round || 0) === (maxRound[mid] || 0); }
        var out = raw.filter(function(e) {
          var to = byId[e.to], from = byId[e.from];
          if (to && to.kind === 'system' && /chair/i.test(to.id) && from && from.kind !== 'system') return lastRound(from);
          return true;
        });
        var chair = null, closeN = null;
        nodes.forEach(function(n) { if (/chair/i.test(n.id) || n.id === 'chairman') chair = n; else if (n.kind === 'system') closeN = n; });
