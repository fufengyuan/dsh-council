  ...
  factory: function(require) {
    ...
    function CouncilPanelContent(props) {
      ...
      function createIssue() {
        ...
        if (!t) return;
        setTopic('');
        fetch('/council/api/progress/report', {
          method:'POST', headers:{'content-type':'application/json'},
          body: JSON.stringify({ problem: t, mode: 'quick', sessionId: props.sessionId || '' })
        }).then(function(r){ return r.json(); }).then(function(d){
          if (d.runId) setSelectedRunId(d.runId);
        }).catch(function(){});
      }
      return e('div', { style: { flex: 1, display: 'flex', flexDirection: 'column', minHeight: 0, overflow: 'hidden' } },
        e('div', { style: { flex: 'none', display: 'flex', flexDirection: isMobile ? 'column' : 'row', alignItems: isMobile ? 'stretch' : 'center', gap: isMobile ? 8 : 10, padding: isMobile ? '10px 12px' : '14px 20px', borderBottom: '1px solid var(--dsw-alias-border-l1, rgba(0,0,0,.06))' } },
          ...
          e('button', { onClick: createIssue, disabled: !topic.trim(),
            style: { height: 38, padding: '0 24px', border: 0, borderRadius: 8, fontSize: 14, fontWeight: 600, cursor: 'pointer',
              background: topic.trim() ? 'var(--dsw-alias-brand-primary, #4a6cf7)' : 'var(--dsw-alias-border-l2, #ccc)', color: '#fff' }
          }, '发起辩论')),
    ...
    function CouncilOverlayPanel(props) {
      ...
      React.useEffect(function() { if (sessionId && isOpen) controller.close(); }, [sessionId]);
      ...
      return e('div', {
        ...
        e('div', { style: {
          ...
          e('button', { onClick: function(){ controller.close(); },
            style: { width: 32, height: 32, border: 0, borderRadius: 8, background: 'transparent', cursor: 'pointer', fontSize: 18, color: 'var(--dsw-alias-label-tertiary, #888)', display: 'grid', placeItems: 'center' }
          }, '✕')),
    ...
    return { apply: apply, inject: ['slots', 'sessions'] };
  }
});
