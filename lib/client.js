  ...
  factory: function(require) {
    ...
    function injectStyles() {
      ...
      st.textContent = [
        ...
        // 设置页左侧导航：shell 给所有外部 settings.section 一律画兜底齿轮，议会这行再带
        // 自己的图标就会「两个图标并排」。registerSettingsNavIcon 标记出本插件那一行后，
        // 这里隐藏齿轮、用 ::before 补一个等宽图标，保持单图标 + 16px 图标节奏。
        '[data-dsh-council-settings-nav] > svg:first-child { display:none; }',
        '[data-dsh-council-settings-nav]::before { content:"🏛️"; flex:none; width:16px; text-align:center; font-size:14px; line-height:1; }'
      ].join('\n');
      document.head.appendChild(st);
    }

    // 设置页导航行的图标适配。
    // dsh 的 settings.section 契约只投影 id/order/label，没有 icon 字段，shell 的
    // navIcon(id) 对内置 id 之外的所有行一律画兜底齿轮；议会这行要显示自己的图标，
    // 就只能事后按 label 文本精确标记本插件那一行，再靠 CSS 换掉齿轮。
    // 依赖 shell 的 DOM 形状（[role="dialog"] nav button），shell 大改版时需复核。
    function registerSettingsNavIcon(label) {
      var MARKER = 'data-dsh-council-settings-nav';
      var queued = false;
      function sync() {
        queued = false;
        ...
        for (var i = 0; i < buttons.length; i++) {
          ...
          else if (button.hasAttribute(MARKER)) button.removeAttribute(MARKER);
        }
      }
      function schedule() {
        // 面板里 DAG 会随辩论进度高频改动 DOM，这里按帧合并，避免每次 mutation 都全量重排查询。
        if (queued) return;
        queued = true;
        if (typeof requestAnimationFrame === 'function') requestAnimationFrame(function () { sync(); });
        else sync();
      }
      // HMR / 重复 apply：只保留一个 observer，旧的先断开。
      if (window.__dshCouncilNavIconObserver) {
        try { window.__dshCouncilNavIconObserver.disconnect(); } catch (_) {}
      }
      sync();
      var observer = new MutationObserver(schedule);
      // 观察 documentElement 而不是 body：client bundle 可能在 body 挂载前就 apply。
      observer.observe(document.documentElement, {
        childList: true, subtree: true, characterData: true
      });
      window.__dshCouncilNavIconObserver = observer;
    }
      ...
      return e(React.Fragment, null,
        ...
        e('div', { className: 'council-nav-wrapper' },
          e('button', {
            ...
            e('span', { className: 'council-nav-icon' }, live ? '🏛️' : '🏛️'),
            e('span', { className: 'council-nav-label' }, '议会'))));
    }

    // 两处对齐问题（都用 headless Chrome 量过像素）：
    // 1) 行几何：shell 的 footer action 行（任务板）是 width:calc(100% + 8px) + margin:4px -4px，
    //    议会原来用 width:100% 且无负边距，整行比邻居右移 4px。
    // 2) 图标：emoji 的行盒宽 22px（比 16px 图标占位宽），又会把文字顶右 4px。
    //    塞进 16×16 的 flex 盒居中、字号 14px（墨迹高 ≈13.5px，与 shell 16px 线性图标齐平）。
    var NAV_STYLES = '.council-nav-wrapper{width:100%;flex:0 0 100%}[class*="footerActions"]{flex-wrap:wrap !important}.council-nav-btn{-webkit-appearance:none;appearance:none;display:flex;align-items:center;gap:8px;width:calc(100% + 8px);height:34px;margin:4px -4px;padding:6px 2px 6px 10px;box-sizing:border-box;border:none;border-radius:12px;background:transparent;color:var(--dsw-alias-label-primary,#0f1115);cursor:pointer;font:inherit;font-size:14px;line-height:22px;overflow:hidden}.council-nav-icon{display:inline-flex;align-items:center;justify-content:center;width:16px;height:16px;flex:none;font-size:14px;line-height:1;overflow:hidden}.council-nav-btn:hover{background:var(--dsw-alias-interactive-bg-hover,rgba(38,49,72,.06))}.council-nav-label{overflow:hidden;white-space:nowrap}';
    ...
    function apply(ctx) {
      ...
      // 3. 设置页顶级菜单：议会配置
      // label 只放纯文本：图标由 registerSettingsNavIcon + CSS 负责。把 emoji 写进 label
      // 会和 shell 的兜底齿轮并排，看起来就是「图标重复了」。
      var councilNavLabel = function() { return '议会'; };
      registerSettingsNavIcon(councilNavLabel);
      ctx.slots.inject('settings.section', function* () {
        yield ctx.slots.register({
          name: 'settings.section',
          id: 'council',
          order: 20,
          label: councilNavLabel
        }, CouncilSettings);
      });
    }
