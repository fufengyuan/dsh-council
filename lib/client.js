  ...
  factory: function(require) {
    ...
    function CouncilPanelContent(props) {
      ...
      React.useEffect(function() {
        ...
        // 优先选中「当前会话且有节点」的 run；否则选有内容（nodes>0）的最近 run；再退回最新
        ...
        var pick = (withData.length > 0 ? withData[0] : runList[0]);
        console.log('[council] select run:', pick.id, 'nodes:', pick.nodes, 'from', runList.length, 'runs; sessionId=', props.sessionId);
        ...
        yield ctx.slots.register({
          ...
          inject: function() { return {
            ...
            createSession: async function(workspaceId, problem) {
              ...
              try {
                // 先打开会话激活 agent，再注入 prompt
                try { ctx.sessions.open(newId); } catch(_) {}
                await new Promise(function(r){ setTimeout(r, 500); });
                var binding = ctx.sessions.binding(newId);
                if (binding && typeof binding.prompt === 'function') {
                  await binding.prompt([{ type: 'text', text: '/council --quick ' + problem }], 'queue');
                } else {
                  console.error('[council] binding unavailable for', newId, 'binding=', binding);
                }
              } catch (e) {
